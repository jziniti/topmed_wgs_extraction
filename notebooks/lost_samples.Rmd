...
setwd("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/")
...
for(study in STUDY){
  ...
  setwd("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/1.cross_study_check/")
...
```
cd /udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/1.cross_study_check
...
write.csv(losts,"/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/New_study_83samples.csv",row.names=F,quote=F)
```
...
```
cd /udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check
...
do.king <- function(race=c("AA","NHW")){
  ...
  setwd("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check/copdgene_genotype")
...
### LTRC
- /proj/regeps/regep00/studies/LTRC/analyses/resaf/Genotypes/Freeze9b_redo/PLINK/biallele_SNP_MAF_0.01_chr*.bim
- hg38

```{r eval=F}
## Extract overlapping variants from LTRC
ext.ovl <- function(chr){
  library(data.table)
  ltrc <- fread(paste0("/proj/regeps/regep00/studies/LTRC/analyses/resaf/Genotypes/Freeze9b_redo/PLINK/biallele_SNP_MAF_0.01_chr",chr,".bim"),head=F,data.table=F)
  lost_sp <- fread(paste0("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check/copd_unknown_83_merge_pruned.bim"),head=F,data.table=F)
  
  ltrc$snpID <- with(ltrc,paste(V1,V4,V5,V6,sep=":"))
  lost_sp$snpID1 <- with(lost_sp,paste(V1,V4,V5,V6,sep=":")) 
  lost_sp$snpID2 <- with(lost_sp,paste(V1,V4,V6,V5,sep=":")) 

  ovl <- ltrc[ltrc$snpID%in%lost_sp$snpID1 | ltrc$snpID%in%lost_sp$snpID2,]
  ovl.bed <- ovl[,c("V1","V4","V4")]
  setwd("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check/LTRC")
  write.table(ovl.bed,paste0("chr",chr,"_plink.bed"),row.names=F,col.names=F,quote=F)
  
  ### Extract overlapping variants
  ## LTRC
  system(paste0("plink2 --bfile /proj/regeps/regep00/studies/LTRC/analyses/resaf/Genotypes/Freeze9b_redo/PLINK/biallele_SNP_MAF_0.01_chr",chr," --extract bed1 chr",chr,"_plink.bed --make-pgen --sort-vars --out LTRC_chr",chr))
  system(paste0("plink2 --pfile LTRC_chr",chr," --make-bed --out LTRC_chr",chr))
  bim <- fread(paste0("LTRC_chr",chr,".bim"),head=F,data.table=F)
  bim$V2 <- with(bim,paste(V1,V4,sep=":"))
  write.table(bim,paste0("LTRC_chr",chr,".bim"),row.names=F,col.names=F,quote=F,sep="\t")
}
setwd("/udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check/LTRC")
aa <- lapply(1:22,ext.ovl)

### Merge LTRC files
for(chr in 2:22){
  system(paste0("echo LTRC_chr",chr," >> LTRC.mergelist"))
}
system("/udd/redpr/bin/plink19 --bfile LTRC_chr1 --merge-list LTRC.mergelist --make-bed --out LTRC_merge")

### Extract overlapping variants from 83 samples
system(paste0("cat ",paste0(paste0("chr",1:22,"_plink.bed"),collapse=" ")," > ovl_plink.bed"))
system("plink2 --bfile /udd/rewki/projects/TOPMed/Freeze.10/Lost_samples/2.indep_study_check/copd_unknown_83_merge_pruned --extract bed1 ovl_plink.bed --make-bed --out 83samples")
bim <- fread("83samples.bim",head=F,data.table=F)
bim$V2 <- with(bim,paste(V1,V4,sep=":"))
write.table(bim,"83samples.bim",row.names=F,col.names=F,quote=F,sep="\t")
  
### Merge LTRC and 83 samples
system("/udd/redpr/bin/plink19 --bfile LTRC_merge --bmerge 83samples --make-bed --out OVL_merge")

### King
system("/udd/redpr/bin/king -b OVL_merge.bed --duplicate --prefix king")

## Remove tmp files
system("rm -f 83samples.*")
system("rm -f chr*")
system("rm -f LTRC*")
system("rm -f OVL_*")
system("rm -f ovl_*")
